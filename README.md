...
>    - For improvements, please review section "What I could have done better or improved" below.
...
>    - However we can render the countries list from our cache until something really changes in the list. 
...
>    - Currently I am defaulting the date to today's date, So having an overloaded method which will accept the custom date should be built and reuse the same logic I have for sorting today. 
...
>    - This would need a distributed counter per country (as we have the applications running on multiple nodes). We can apply the CRDT (Conflict-free replicated Datatype) pattern to our population service, which helps us create a distributed map of separate counters for each country and communicate this distributed map to all its peers in the consistent-hash ring at the same time. This can get us the real time counts for the countries in demand. 
...
Please run the below command to instantiate the architecture shown above as docker containers.
...
This layer contains the code in the repo and does the business logic required

#### Cache Layer
Two Levels of cache has been added. 
...
Reason for this level of cache is to reduce the burden on Redis instance which we are using as distributed cache. 
- Redis for full level of distributed caching, which acts as fall back layer for in memory cache and the actual country api which you provided is the fall back for Redis. 

#### Note: Having this kind of cache mechanism will increase the resilency and would reduce the calls to 3rd party servies significantly. 
...
- cacheservice has all the files required for Cache Layer describe above. 
  - 2 Classes 
    - Cache.js : Entry point to the cache service.
    - RedisCacheService.js : Has all the required code for connecting to Redis instance when available. 

- httpservice has files required to make fetch calls to the external country service. 
  - fetchUrl (to fetch 1 url) 
  - fetchUrls (to fetch multiple urls at once).
...
- index.js : Routing path definition 

#### Package 'lib'
Added population-helper.js class which has the logic to fetch population based on the countries list sent in the request.
...
### What I could have done better or improved. 
...
- Security of the application adding https, and some level of authorization by adding a Gateway in front of the NGINX service.
  - Simple one would be users registering for this service will get a api key and use this for calling service.
